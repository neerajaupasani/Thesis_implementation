...
class Cora:
    ...
    def random_walk(self, path_length, start=None):
        ...
        with open('cora_paths.txt', 'w+') as f:
            for item in path:
                f.write("%i\n" % item)
    ...
    def similarity_scores(self):
        files = ['cora_cosine.txt', 'cora_jaccard.txt', 'cora_lhn1.txt']
    ...
    def similarity_df(self):
        global uni_df
        uni_df = pd.read_csv('output_file.txt', header = None)
        ...
        uni_df.to_csv(r'cora_similarities.csv')
    ...
    def cosine_mat(self, dimension, output):
        ...
        cos_mat.to_csv(r'cosine_matrix.csv')
        ...
        #cos_mat = cos_mat.reset_index()

        #with open('cora_vocab.txt', 'w') as f:
            #for i in cos_mat['node1'].tolist():
                #f.write("%i\n" %i)

        cora_df = pd.read_csv('cosine_matrix.csv')
    ...
    def jaccard_mat(self, dimension, output):
        ...
        jac_mat.to_csv(r'jaccard_matrix.csv')
        jac_array = jac_mat.values
        #global jaccard_mat
        jaccard_mat = coo_matrix(jac_array)
        cora_df = pd.read_csv('jaccard_matrix.csv')
    ...
    def lhn1_mat(self, dimension, output):
        ...
        lhn1_mat.to_csv(r'lhn1_matrix.csv')

        lhn1_array = lhn1_mat.values
        #global lhn1_coomat

        lhn1_coomat = coo_matrix(lhn1_array)
        cora_df = pd.read_csv('lhn1_matrix.csv')
...
obj.similarity_df()
obj.cosine_mat(16,'cosine_embeddings.csv')

obj.jaccard_mat(16,'jaccard_embeddings.csv')

obj.lhn1_mat(16,'lhn1_embeddings.csv')
