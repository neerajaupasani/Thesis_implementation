...
class Cora_graph:
    ...
    def read_graph(self, input):
        ...
        print("The number of nodes are : %d, \n The number of edges are: %d" % (G.number_of_nodes(), G.number_of_edges()))
    ...
    def generate_pos_neg_links(self):
        ...
        print("Found %d of %d non-edges" % (nneg, len(non_edges)))
        ...
        if len(neg_edge_list) < nneg:
            print("There are only %d negative edges" % (len(neg_edge_list)))
            #raise RuntimeWarning(
                #"Only %d negative edges found" % (len(neg_edge_list))
            #)

        print("The positive edges of the total edges are %d" % npos)
...
class Cora_linkprediction():

    def __init__(self, embeddings, edge_file):
        self.embeddings = embeddings
        self.edge_file = edge_file

    def create_train_test_graphs(self, input='cora-edgelist.txt'):
        ...
        Gtrain = Cora_graph(is_directed=False,
                       prop_pos=prop_pos,
                       prop_neg=prop_neg)
        ...
        Gtest = Cora_graph(is_directed=False,
                      prop_pos=prop_pos,
                      prop_neg=prop_neg)
    ...
    def test_edge_functions(self, num_experiments=2, emb_size=128, model=None, edges_train=None, edges_test=None,
                            Gtrain=None, Gtest=None, labels_train=None, labels_test=None):
        ...
        for iter in range(num_experiments):
            ...
            for edge_fn_name, edge_fn in edge_functions.items():
                ...
                AUC = roc_auc_scorer(clf, edge_features_test, labels_test)
    ...
    def predict(self, size):
        ...
        Gtrain, Gtest = self.create_train_test_graphs(self.edge_file)
        ...
        auc = self.test_edge_functions(2, size, self.embeddings, edges_train, edges_test, Gtrain, Gtest, labels_train,
                                       labels_test)
