...
class Citeseer:
    ...
    def random_walk(self, path_length, start=None):
        ...
        with open('cite_paths.txt', 'w+') as f:
            for item in path:
                f.write("%i\n" % item)
    ...
    def similarity_scores(self):
        files = ['cite_cosine.txt', 'cite_jaccard.txt', 'cite_lhn1.txt']
        with open('output_file_cite.txt', 'w+') as output:
    ...
    def similarity_df(self):
        global uni_df
        uni_df = pd.read_csv('output_file_cite.txt', header = None)
        ...
        uni_df.to_csv(r'cite_similarities.csv')
    ...
    def cosine_mat(self, dimension, output):
        ...
        cos_mat.to_csv(r'cosine_matrix_cite.csv')
        cos_array = cos_mat.values

        #cos_array[cos_array > 0.25] = 0
        #global cosine_mat
        cosine_mat = coo_matrix(cos_array)
        cora_df = pd.read_csv('cosine_matrix_cite.csv')
        #global cosine_mat
    ...
    def jaccard_mat(self, dimension, output):
        ...
        jac_mat.to_csv(r'jaccard_matrix_cite.csv')
        jac_array = jac_mat.values
        #global jaccard_mat
        jaccard_mat = coo_matrix(jac_array)
        cora_df = pd.read_csv('jaccard_matrix_cite.csv')
    ...
    def lhn1_mat(self, dimension, output):
        ...
        lhn1_mat.to_csv(r'lhn1_matrix_cite.csv')

        lhn1_array = lhn1_mat.values
        #global lhn1_coomat

        lhn1_coomat = coo_matrix(lhn1_array)
        cora_df = pd.read_csv('lhn1_matrix_cite.csv')
...
obj.similarity_df()
obj.cosine_mat(16,'cosine_embeddings_cite.csv')
obj.jaccard_mat(16,'jaccard_embeddings_cite.csv')
obj.lhn1_mat(16,'lhn1_embeddings_cite.csv')
