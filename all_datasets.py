...
class Cora:
    ...
    def node_pairs(self, window):
        ...
        for win in windows:
            ...
            for i in range(window * 2 + 1):
                ...
                pairs.append((u, v, cosine_uv, jaccard_uv, hpi_uv, hdi_uv))
    ...
    def temp_pmi(self, pmi_threshold, output):
        pmi = self.pmi_mat(pmi_threshold)
        U = TruncatedSVD(n_components=32, algorithm="arpack")
        t1 = time.time()
    ...
    def temp_cosine(self, output):
        global cosine
        U = TruncatedSVD(n_components=8, algorithm="arpack")
        t1=time.time()
